...
function App() {
    ...
    setTripReview((prev) => ({
      trips: prev.trips.map((trip) => {
        ...
        const nextSegment: RouteSegment = {
          ...
          preference: payload.routeType === 'driving' ? payload.preference ?? 'HIGHWAY_FIRST' : undefined,
  ...
  const routeTypeValue: RouteType = activeSegment?.routeType ?? 'driving'
  const routePreferenceValue: RoutePreference = activeSegment?.preference === 'AVOID_TOLL' ? 'AVOID_TOLL' : 'HIGHWAY_FIRST'
    ...
    <main className="app-shell">
      ...
      <MapPlaceholder
        ...
        onChangeRouteType={(value) => {
          ...
          updateSegment(activeSegmentId, (segment) => ({
            ...segment,
            routeType: value,
            preference: value === 'driving' ? segment.preference ?? 'HIGHWAY_FIRST' : undefined,
          }))
        }}
        onChangeRoutePreference={(value) => {
          if (!activeSegmentId) return
          updateSegment(activeSegmentId, (segment) => ({ ...segment, preference: value, routeType: 'driving' }))
        }}
