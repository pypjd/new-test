...
import { routePreferenceOptions, routeTypeOptions } from '../utils/routePreference'
...
function TripEditor({ trips, onAddTrip, onAddSegment }: TripEditorProps) {
  ...
  const handleAddSegment = (event: FormEvent<HTMLFormElement>) => {
    ...
    onAddSegment({
      ...
      preference: segmentRouteType === 'driving' ? segmentPreference : undefined,
  ...
  return (
    <section className="card-section">
      ...
      <form className="form-block" onSubmit={handleAddSegment}>
        ...
        <input value={segmentViaPointsText} onChange={(e) => setSegmentViaPointsText(e.target.value)} placeholder="途径点（逗号分隔）" />
        ...
        {segmentRouteType === 'driving' && (
          <label>
            路线策略
            <select value={segmentPreference} onChange={(e) => setSegmentPreference(e.target.value as RoutePreference)}>
